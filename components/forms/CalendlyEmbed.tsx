...
/**
 * Inline Calendly scheduler. Rendered after mount because it needs the page's
 * hostname and theme. The event details Calendly prints itself (host name,
 * event title, duration) are hidden: the card around it says the same in the
 * site's own type, and the host's Calendly profile name shouldn't show here.
 */
export function CalendlyEmbed({ url }: { url: string }) {
  ...
  useEffect(() => {
    const build = () =>
      setSrc(
        `${url}?embed_domain=${encodeURIComponent(window.location.hostname)}&embed_type=Inline&hide_gdpr_banner=1&hide_event_type_details=1&${themeParams()}`,
      );
    build();
